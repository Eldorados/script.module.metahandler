...
class TMDB(object):
    ...
    def _do_request(self, method, values):
        ...
        try:
            meta = json.loads(net.http_GET(url, {"Accept": "application/json"}).content)
    ...
    def _convert_date(self, string, in_format, out_format):
        ...
        strptime = lambda date_string, format: datetime(*(time.strptime(date_string, format)[0:6]))  # noqa
        try:
    ...
    def search_imdb(self, name, imdb_id='', year=''):
        ...
        if self.omdb_api_key:
            ...
            try:
                ...
                meta = json.loads(net.http_GET(url).content)
    ...
    def tmdb_search(self, name):
        ...
        Returns:
            DICT of matches
        '''
        return self._do_request_all('search/movie', 'query=' + urllib_parse.quote(name))
        ...
        if not imdb_id and not tmdb_id:
            ...
            if meta and meta['total_results'] == 1 and meta['results']:
                ...
                for result in meta['results']:
                    if result.get('title') == name and result.get('release_date', '0000')[:4] == year:
                        tmdb_id = result.get('id')
                        if 'imdb_id' in result:
                            imdb_id = result.get('imdb_id')
                if tmdb_id == '':
                    for result in meta['results']:
                        if result.get('title') == name:
                            tmdb_id = result.get('id')
                            if 'imdb_id' in result:
                                imdb_id = result.get('imdb_id')
