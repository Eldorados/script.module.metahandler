...
class MetaData:
    ...
    def __format_meta(self, media_type, meta, name):
        ...
        try:
            ...
            if 'meta_data' in meta.keys():
                meta = meta.get('meta_data')

            meta['title'] = name

            # Change cast back into a tuple
            if meta['cast']:
                meta['cast'] = json.loads(meta['cast'])
            ...
            try:
                ...
                if trailer_id:
                    meta['trailer'] = 'plugin://plugin.video.youtube/play/?video_id=%s' % trailer_id
    ...
    def update_meta(self, media_type, name, imdb_id, tmdb_id='', new_imdb_id='', new_tmdb_id='', year=''):
        ...
        '''
        logger.log('---------------------------------------------------------------------------------------')
        logger.log('Updating meta data: %s Old: %s %s New: %s %s Year: %s' % (name.encode('utf-8') if six.PY2 else name, imdb_id, tmdb_id, new_imdb_id, new_tmdb_id, year))
    ...
    def _cache_save_video_meta(self, meta_group, name, media_type, overlay=6):
        ...
        for meta in meta_group:
            ...
            if 'cast' in meta:
                meta['cast'] = json.dumps(meta['cast'])
            ...
            if media_type == constants.type_movie and meta['imdb_id'] and meta['tmdb_id'] and meta['title'] and meta['year']:
                ...
                logger.log('SQL INSERT: %s' % sql_insert)
                values = (
                    meta['imdb_id'], meta['tvdb_id'], meta['title'], meta['year'],
